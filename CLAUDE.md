...
* Follow the rules as defined in this explicit rule index:
  ...
  * @rules/General.md `[CRITICAL]` — global baseline behavior: assumptions, validation discipline, safety, exec-context routing, upstream-contract verification, commit ticket traceability, output-language policy, skill-invocation gate, sub-agent delegation, git workflow. Re-read on every revalidation (§3.4).
...
* **Core skills** — auto-activate on prompt relevance, or invoke explicitly:
  ...
  * `/core:composer` — Composer resolution order, dev-overrides, canonical-priority trap, lock-file discipline.
...
* **TYPO3 workflow skills** — explicit activation required:
  * `/typo3:upgrade` — Upgrade Workflow (execution + DoD); v10–v14 changelog + migration patterns in `references/`.
