...
* **Core workflow skills** — activation policy per entry; see each skill's description for trigger patterns:
  * `/core:batch` (`~/.claude/plugins/marketplaces/local/plugins/core/skills/batch/`) — auto-suggest gate: Claude interrupts and proposes activation on trigger match (incl. `General.md` §3.5 scope growth) but never silently runs it. Shared batch execution template, risk-sequenced Pass 1/2/3 model + triage/PR-split gates, autonomous-mode protocol, chaining model, reporting. Foundation for the typo3 workflow skills.
