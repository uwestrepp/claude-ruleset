...
* Follow the rules as defined in this explicit rule index:
  ...
  * rules/Skills.md (path-gated: `**/SKILL.md`, `**/agents/*.md`) — description authoring: a description IS the activation trigger; paraphrase and restraint margins are per skill and measured, not assumed; a changed description needs a second input SHAPE tested via `claude plugin eval`, whole-suite (over-trigger risk).
