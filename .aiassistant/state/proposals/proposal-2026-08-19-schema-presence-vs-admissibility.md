...
Date:         2026-08-19
Status:       open
Origin:       session observation — two consecutive failures while seeding the Shopify
              store schema in the MO project
Revisit when: a decision on this proposal, or a third instance of "the schema declares it,
              so I may use it here" in any project
```

## Problem

While writing the Shopify seed script I did what §1.4 and §2.2 ask for: I did not write
API field names from memory, I introspected the live GraphQL schema first. Two of the
...
- **§2.2** is the closest fit and already speaks of the "effective runtime/parser", but it
  frames verification as *version and toolchain* compatibility. Both failures were inside
  one confirmed API version, with a correct introspection in hand. Nothing in the section
...
Prevents exactly the two failed runs above and the false confidence that produced them.
...
- Partly a model-default in the sense that a careful reader might infer it. The two
...
- Both failures and their corrected form are recorded in `docs/04-datenmodell.md` §4.8,
  point 1 (`ÜBERHOLT`, admin access) and point 5 (reference validations), and in
  `store-schema/README.md` points 1 and 2.
