---
name: git-knowledge
description: "Use for git branch/workflow mechanics beyond the General.md §12 safety baseline: detecting a project's real deploy mapping, disambiguating remotes/worktrees/upstreams, choosing a diff or merge baseline, rebase/merge/force-push hygiene, recovering lost work. Triggers: \"which branch does this deploy\", \"does merging to main deploy\", CI deploy-mapping questions (bitbucket-pipelines.yml / .gitlab-ci.yml / GitHub Actions branch filters), multiple remotes (origin vs fork vs deploy mirror), git worktree setup, choosing a PR/diff/upgrade baseline, \"undo a bad reset / recover a deleted branch / find a lost commit\", reflog recovery, force-push safety, interactive-rebase alternatives. NOT commit-message drafting (/core:commits), NOT git-hook install (/core:githooks-install)."
argument-hint: "[topic]"
