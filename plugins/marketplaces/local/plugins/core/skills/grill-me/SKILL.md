---
name: grill-me
description: "Activate via /core:grill-me or let Claude auto-activate when the user asks to pressure-test, interrogate, or poke holes in a plan/design BEFORE building. Adversarial requirements-elicitation primitive: enumerate a plan's material decision points, order them by dependency, ground each in the codebase first, then interview the user only on what code can't answer — until no unresolved decision would still change the design. Ends by emitting a written decision record (resolved choices, stated assumptions, rejected alternatives). Triggers: '/core:grill-me', 'grill me on', 'poke holes in my plan', 'interview me about this design', 'pressure-test this approach', 'what am I missing', 'challenge my assumptions', 'stress-test the plan before we build'. DOES NOT auto-trigger on routine task start, on 'implement/fix/refactor X', or on a converged plan the user has already decided to execute — interrogation is high-friction and fires only on explicit plan-critique intent. Complementary to advisor (one strong opinion on your direction) and /core:brainstorm (N divergent candidates) — grill-me converges a single plan by resolving its uncertainties."
...
Invoke when the user wants a plan stress-tested before execution:
- "poke holes in this plan / design",
