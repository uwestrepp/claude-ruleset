---
name: grill-me
description: "Activate via /core:grill-me or let Claude auto-activate when the user asks to pressure-test or interrogate a plan/design they are still forming, BEFORE building — by being interviewed, not by receiving a critique. Adversarial requirements-elicitation primitive: enumerate a plan's material decision points, order them by dependency, ground each in the codebase first, then interview the user only on what code can't answer — until no unresolved decision would still change the design. Ends by emitting a written decision record (resolved choices, stated assumptions, rejected alternatives). Triggers: '/core:grill-me', 'grill me on', 'help me firm up my plan', 'interview me about this design', 'pressure-test this approach', 'what am I missing', 'challenge my assumptions', 'stress-test the plan before we build'. DOES NOT auto-trigger on routine task start, on 'implement/fix/refactor X', or on a converged plan the user has already decided to execute — interrogation is high-friction and fires only on explicit plan-critique intent. Complementary to advisor (one strong opinion on your direction) and /core:brainstorm (N divergent candidates) — grill-me converges a single plan by resolving its uncertainties. Distinct from /core:poke-holes, which attacks an already-supplied artifact and returns findings WITHOUT interviewing — use that when an artifact exists and no Q&A is wanted; use grill-me to interview a plan you are still forming."
