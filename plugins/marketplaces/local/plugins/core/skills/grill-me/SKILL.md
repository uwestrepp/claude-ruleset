---
name: grill-me
description: "Use when the user asks to pressure-test or interrogate a plan/design they are still forming, BEFORE building — by being interviewed, not by receiving a critique. Triggers: 'grill me on', 'help me firm up my plan', 'interview me about this design', 'pressure-test this approach', 'what am I missing', 'challenge my assumptions', 'stress-test the plan before we build'. DOES NOT auto-trigger on routine task start, on 'implement/fix/refactor X', or on a converged plan the user has already decided to execute. Distinct from /core:poke-holes (attacks an already-supplied artifact and returns findings WITHOUT interviewing — use that when an artifact exists and no Q&A is wanted) and /core:brainstorm (generates N divergent candidates) — grill-me interviews to converge a single plan."
