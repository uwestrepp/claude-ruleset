---
name: poke-holes
description: "Activate via /core:poke-holes or let Claude auto-activate when the user supplies an artifact (plan, proposal, design, assumption-set, spec, or implementation — their own, a colleague's, or the agent's) and asks to critique it, find its flaws, or stress-test it WITHOUT being interviewed. Adversarial, artifact-agnostic critique primitive: attacks a GIVEN artifact and returns a severity-ranked findings list (Blocking / Material) — never nitpicks, never asks questions, never proposes alternatives. Materiality is the floor: nothing below Material is reported. Default Tier 1 is a cheap blocking-only pass; Tier 2 (on request) adds a multi-lens panel; an optional steelman mode attacks only the strongest reading. Triggers: '/core:poke-holes', 'poke holes in this <doc/proposal/design>', 'critique this', 'find the flaws in this', 'what's wrong with this plan', 'tear this apart', 'any showstoppers in this?', 'red-team this design'. DOES NOT auto-trigger on routine task phrasing, on 'improve/implement/refactor X', or when no concrete artifact is supplied. Distinct from /core:grill-me (which INTERVIEWS you to converge a plan you are about to build — poke-holes asks nothing and converges nothing), from /core:brainstorm (which GENERATES alternatives), and from code-review/pr-review tools (which do line-level CODE-diff review — poke-holes is stack/format-agnostic and operates at the design/claim level). On genuine ambiguity with grill-me, disambiguate before invoking."
