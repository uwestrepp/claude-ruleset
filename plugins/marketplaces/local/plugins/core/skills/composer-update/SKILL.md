...
description: "Activate via /core:composer-update (optionally with an ecosystem hint, e.g. /core:composer-update typo3) before running a non-trivial Composer update in a customized project — security patch, patch/minor bump, or any update where project customizations could collide with what the update changes. Provides the informed-update workflow: capture baseline → resolve the exact per-package version delta (dry-run) → build a reusable upstream change-map → intersect it with the project's own customizations using an ecosystem collision-vector catalog → document → only then run the gated rollout. Layers on /core:batch (phase/Pass model) and references /core:composer (resolution/lock mechanics) and /core:commits. The collision-vector knowledge is pluggable catalog data (references/catalogs/<ecosystem>.md); the skill self-detects the ecosystem and, when no catalog exists for it, falls back to the generic catalog and offers to author a new one. Ships with a TYPO3 catalog. explicit activation required — auto-suggest (do not auto-run) when a request is to run composer update/require/bump on a project with local packages, patches, XCLASS/decoration/overrides, or a security advisory, especially across multiple projects. Triggers: 'safely update <package>', 'roll out a security update', 'composer update without breaking overrides', 'will this update break our customizations', 'bump TYPO3/Symfony/Shopware patch level', 'check before composer update', 'apply CVE/security advisory fix across projects'. NOT for trivial adds in vanilla projects (use /core:composer) and NOT for major-version migrations (use /typo3:upgrade for TYPO3)."
