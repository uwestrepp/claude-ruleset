---
name: brainstorm
description: "Activate via /core:brainstorm or let Claude auto-activate when the user explicitly asks for diversity/alternatives/hypotheses. Verbalized-sampling brainstorming primitive for design alternatives, debugging hypotheses, test-case diversity, and code-review angles. Delegates generation of N distinct candidate approaches (with rationale and confidence) to a sub-agent, then condenses to a ranked shortlist using a viability/distinctness/fit/cost rubric. Triggers: '/core:brainstorm', 'brainstorm N approaches/alternatives/hypotheses', 'give me distinct ways to', 'list debugging hypotheses for', 'explore design alternatives for', 'what are different angles for', 'use verbalized sampling'. DOES NOT trigger on generic 'how do I solve X' or routine task-execution phrasing — this skill is for genuine exploration where diversity is the goal, not for converged decisions where the agent should answer directly. Also NOT for deciding one module's interface/API shape by comparing concrete designs, and NOT for building runnable prototypes — brainstorm returns a ranked list of candidate approaches, not designs or code. Complementary to the advisor pattern (one strong opinion vs. N diverse candidates judged on a rubric)."
