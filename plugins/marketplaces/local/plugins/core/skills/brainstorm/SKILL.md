---
name: brainstorm
description: "Activate via /core:brainstorm or let Claude auto-activate when the user explicitly asks for diversity/alternatives/hypotheses — genuine exploration where diversity is the goal. Triggers: '/core:brainstorm', 'brainstorm N approaches/alternatives/hypotheses', 'give me distinct ways to', 'list debugging hypotheses for', 'explore design alternatives for', 'what are different angles for', 'use verbalized sampling'. DOES NOT trigger on generic 'how do I solve X' or routine task-execution phrasing, nor on converged decisions where the agent should answer directly. Also NOT for deciding one module's interface/API shape by comparing concrete designs, and NOT for building runnable prototypes — brainstorm returns a ranked list of candidate approaches, not designs or code."
