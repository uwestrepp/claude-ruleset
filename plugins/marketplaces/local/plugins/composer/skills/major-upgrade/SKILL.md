---
name: major-upgrade
description: "Activate via /composer:major-upgrade (optional ecosystem/target hint, e.g. typo3 13) before any major-version upgrade of a framework or platform managed through Composer — a jump that crosses a deprecation/breaking-change line (e.g. cms-core ^12→^13, symfony 6→7, shopware 6.5→6.6), not a patch/minor bump. System-agnostic spine, a faithful specialization of /core:batch §1 (phases 0–9): branch/baseline → changeset + collision map → deprecation/breaking scan → staged implementation (cleanup-on-old → version bump + boot fixes → framework remediation → gated destructive schema/data) → before/after validation → rollout doc + commits → (decoupled) runtime bump as a post-rollout follow-up. Layers on /core:batch; reuses /composer:update's delta/collision mechanics; references /composer:knowledge and /core:commits. Framework specifics come from a pluggable slot contract (references/framework-slot-contract.md); /typo3:upgrade is the TYPO3 specialization. explicit activation required — auto-suggest (do not auto-run) on requests like 'upgrade <framework> from major X to Y', 'major version migration', 'bump cms-core to the next major'. NOT patch/minor/security updates (/composer:update); NOT resolution/lock questions (/composer:knowledge)."
