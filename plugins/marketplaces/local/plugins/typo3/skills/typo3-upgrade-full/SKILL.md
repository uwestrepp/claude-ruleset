...
description: "Activate with /typo3:typo3-upgrade-full to run all three TYPO3 upgrade workflows consecutively in a single chained session (upgrade execution → ExtensionScanner → static code tests). Also activate /typo3:typo3-upgrade, /typo3:typo3-scanner, and /typo3:typo3-static-tests in the same session to make all workflow content available. Applies Batch.md §6 chaining model with a single toolset gate, single preflight, and one final Phase 9 handover for the full chain."
...
- `/typo3:typo3-upgrade-full` — this orchestration skill
- `/typo3:typo3-upgrade` — upgrade execution + DoD content
- `/typo3:typo3-scanner` — ExtensionScanner workflow content
- `/typo3:typo3-static-tests` — static code test workflow content
